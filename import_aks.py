...
class AKSImportSetUp:
    ...
    def __init__(self, subscription_id, region, resource, local_repo_path, filters):
        self.aks_client = Utilities.create_client(subscription_id = subscription_id, resource=resource,)
    ...
    def describe_aks_cluster(self):
        ...
        for rg in resource_groups:
            ...
            for cluster in clusters:
                ...
                cluster_detail = self.aks_client.managed_clusters.get(rg.name, cluster.name)
    ...
    def generate_import_blocks(self, aks_cluster_details):
        ...
        for aks_cluster in aks_cluster_details:
            ...
            context = {"cluster_name": aks_cluster["cluster_name"], "subscription_id": self.subscription_id, "resource_group": aks_cluster['resource_group'], "node_pools": aks_cluster['node_pools'] }

            rendered_template = template.render(context)

            output_file_path = f"{self.local_repo_path}/import-{aks_cluster['cluster_name']}.tf"
            with open(output_file_path, "w") as f:
                f.write(rendered_template)

            Utilities.run_terraform_cmd(["terraform", f"-chdir={self.local_repo_path}", "plan", f"-generate-config-out=generated-plan-import-{aks_cluster['cluster_name']}.tf"])
            os.rename(output_file_path, f"{output_file_path}.imported")
            cleanup_tf_plan_file(input_tf_file=f"{self.local_repo_path}/generated-plan-import-{aks_cluster['cluster_name']}.tf")
            exit(1)
